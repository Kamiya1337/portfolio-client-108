...
export default function RubricTable() {
  ...
  const completedMilestones = projects.reduce((count, project) => count + Number(valid(project.report)), 0);
  const progressPercent = Math.round((completedMilestones / totalMilestones) * 100);
  const projectRows = projects.map((project) => {
    const reportDone = valid(project.report);
    return { id: project.id, title: `${project.title.split(':')[0]}: Báo cáo học phần`, standard: 'Đầy đủ file báo cáo PDF chi tiết nội dung học tập thực tế.', complete: reportDone, action: reportDone ? 'Đã hoàn thiện' : 'Cần bổ sung file báo cáo PDF' };
  });
