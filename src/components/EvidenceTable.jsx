...
export default function EvidenceTable() {
  ...
  const StatusBadge = ({ project }) => {
    ...
    return complete ? <span className="ledger-status ledger-status-complete"><Check size={12} /> Đã nộp</span> : <span className="ledger-status">Thiếu báo cáo</span>;
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="page-intro">
        ...
        <p className="page-copy">Hệ thống hóa toàn bộ các tệp báo cáo PDF phục vụ cho công tác đối soát học thuật.</p>
      </header>
        ...
        <div className="mx-auto grid gap-8 lg:grid-cols-[1fr_340px] lg:items-start">
          ...
          <div className="divide-y divide-primary/10 md:hidden">
            {projects.map((project) => (
              <article key={project.id} className="py-6 px-5">
                ...
                <div className="mt-6 flex items-center justify-between">
                  <EvidenceLink value={project.report} label="Xem báo cáo PDF" type="pdf" />
                </div>
              </article>
            ))}
          </div>
            ...
            <table className="w-full min-w-[700px] border-collapse text-left">
              <thead>
                <tr className="border-b border-white/50 bg-primary/5 text-[11px] font-semibold uppercase tracking-[0.16em] text-secondary">
                  ...
                  <th className="px-8 py-6 text-center">Report File (PDF)</th>
              ...
              <tbody className="divide-y divide-primary/10">
                {projects.map((project) => (
                  <tr key={project.id} className="group transition-all hover:bg-white/35">
                    ...
                    <td className="px-8 py-8 text-center"><EvidenceLink value={project.report} label="Xem báo cáo PDF" type="pdf" /></td>
                    <td className="px-8 py-8 text-right"><StatusBadge project={project} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="paper-plate p-8">
            ...
            <div className="mt-8 flex gap-10">
              <div><p className="font-display text-5xl text-primary">{projects.length}</p><p className="text-sm text-muted-dark">Báo cáo PDF</p></div>
              <div className="border-l border-primary/10 pl-10"><p className="font-display text-5xl text-primary">{completed}/{projects.length}</p><p className="text-sm text-muted-dark">Hoàn thành</p></div>
            </div>
            <p className="mt-10 border-t border-primary/10 pt-6 text-sm italic text-muted-dark">Đã hoàn tất kiểm chứng tài liệu...</p>
          </div>
          <div className="paper-plate border-t-4 border-t-sunflower-yellow p-8">
            ...
            <h2 className="mt-5 font-display text-3xl font-semibold text-primary">Tỷ lệ nộp bài</h2>
            <p className="mt-5 max-w-xl text-sm leading-7 text-muted-dark">Mỗi trạng thái được tính trực tiếp từ dữ liệu báo cáo PDF hiện có.</p>
